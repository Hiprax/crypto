...
- `src/crypto-manager.ts` — import `MAX_ARGON2_MEMORY_COST`, `MAX_ARGON2_TIME_COST`, `MAX_ARGON2_PARALLELISM`, `MAX_PBKDF2_ITERATIONS` from `./format.js`; add upper-bound checks in constructor; harden `encryptFile` `writeChunk` helper and add persistent `onStreamError` guard.
