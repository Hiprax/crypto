...
- **Wrong-password and tamper decryption tests now assert specific error codes and types** (`src/__tests__/crypto-manager.test.ts`). Previously, tests in the `decryptText / decryptTextSync / decryptFile / decryptFileSync - wrong password` describe blocks and the `encryptData/decryptData roundtrip` describe block only asserted `toThrow(CryptoError)` — no check on `.code` or `.type`. The tests now use a `try/catch` pattern and assert the exact `CryptoError` properties produced by each code path: `code: 'DECRYPTION_FAILED'` + `type: CryptoErrorType.DECRYPTION_FAILED` for text paths and `decryptData` (where the `CryptoError` thrown by `decryptData` is re-thrown unchanged by the outer catch); `code: 'FILE_DECRYPTION_FAILED'` + `type: CryptoErrorType.DECRYPTION_FAILED` for `decryptFile` (where the native GCM error from the streaming pipeline is wrapped); and `code: 'SYNC_FILE_DECRYPTION_FAILED'` + `type: CryptoErrorType.DECRYPTION_FAILED` for `decryptFileSync` (same wrapping pattern). Affected tests: wrong-password async text, wrong-password sync text, wrong-password async file, tampered async file, wrong-password sync file, tampered sync file, wrong key (decryptData), wrong IV (decryptData), tampered ciphertext (decryptData), tampered tag (decryptData).
- **New fast-check sync-file round-trip property** (`src/__tests__/property.test.ts`). Added a property-based test in the `round-trip invariants` describe block that asserts `decryptFileSync(encryptFileSync(buf, p), p) === buf` for arbitrary plaintext buffers and valid `pbkdf2Iterations`. Three size classes are exercised in a single property via `fc.oneof` (1:3:1 weights): empty (0 bytes), sub-chunk (1–65535 bytes, below the 64 KiB `SYNC_ENCRYPT_CHUNK_SIZE`), and multi-chunk (65537–131073 bytes, requiring ≥ 2 full loop iterations). `pbkdf2Iterations` is varied over `fc.integer({ min: 1_000, max: 10_000 })` — within the wire-format bounds and low enough to keep the suite fast. A new `CryptoManager` is created per run at the low-cost Argon2 config (`memoryCost: 2^14, timeCost: 1`) with the sampled iteration count; 20 `numRuns` with `endOnFailure: true`. Follows the suite's `TEST_DIR` conventions for per-property scratch isolation and cleanup.
