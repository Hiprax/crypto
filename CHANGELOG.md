...
- **One command runs every gate: `npm run verify`** (`package.json`, `jest.config.js`). It chains `lint`, `type-check`, `build`, `test`, `check:browser`, `check:types:browser` and `check:exports`, stopping at the first failure, and `prepublishOnly` is now defined as exactly `npm run verify` so the release gate and the gate a working session runs cannot drift apart. The composed chain is identical to what `prepublishOnly` already ran, so nothing was added to or removed from the publish gauntlet. Alongside it, the Jest `coverageThreshold.global` floor was ratcheted from 80/80/80/80 to the measured values floored to integers (statements 95, branches 87, functions 97, lines 95, over 25 suites / 1106 tests); the thresholds move up only, and no `collectCoverageFrom` exclusion was added to reach them. The three gate tiers (FAST, FULL, BROWSER), their exact commands and their measured wall-clock durations are recorded in the repository's contributor documentation, and a new `src/__tests__/gate-surface.test.ts` pins the gate surface itself: it fails if a gate leaves the `verify` chain, if `prepublishOnly` stops delegating to it, if the chain gains a `;` or `||` that would let a failing gate pass, or if a coverage threshold is lowered or a `collectCoverageFrom` exclusion added.
