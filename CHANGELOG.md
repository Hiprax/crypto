...
source edit in this release is a comment. It ships as a release rather than sitting in the
repository because `README.md` is bundled in the npm tarball and rendered on the package
page, so a documentation fix is only actually delivered by publishing it. The `dist/`
artifacts do differ from 1.6.0, because `removeComments` is `false` and the corrected JSDoc
is emitted into the `.js` and `.d.ts` files; nothing executable changed.

`README.md` and the project's internal reference were audited against the source line by
line and across files, then reviewed twice more, and the corrections below are what
survived. Most of them share a shape worth naming: the document stated the **right outcome
via the wrong mechanism**. That kind of defect survives casual review because the visible
claim is true, and it then misleads whoever next changes the code.

One entry below was not found by that audit at all: `check:tarball` turned out to be
incompatible with npm 12 and blocked this very release on its first publish attempt. It is
recorded here because the artifact it gates is the thing being published.

### Added

- **The published tarball is gated rather than merely listed** (`scripts/check-tarball.mjs`, `package.json`, `.github/workflows/release.yml`). `npm pack --dry-run` asserts nothing on its own — it exits 0 for any contents and fails only if packing itself errors — so the release step that claimed to "catch accidental inclusion of source maps, .env files, fixtures, etc." was checking that npm works, not that the artifact was right. The new `npm run check:tarball` asks the installed npm what it would actually pack and then gates it in both directions. **Floor:** every path the `exports` map resolves to, plus `package.json` and the four documents named in `files`, must be present — derived from `package.json` at run time, so an exports map that grows a subpath is covered automatically and one that points at an unpacked file fails here. That is a broken-on-arrival publish neither `publint` nor `attw` can see, because both inspect the working tree rather than the packed file list. **Ceiling:** no packed path may match a source, test, benchmark, CI, tooling-config, lockfile, env, credential, source-map or editor/backup pattern. The sharpest case it closes is npm's packlist force-include list, which overrides both the `files` allowlist and `.npmignore` for any root file whose stem is `readme`, `license`, `licence` or `copying`: a scratch `README.md.tmp` left in the repo root ships to the registry, and no configuration suppresses it. It pins **no file count** by design — a count goes red on every legitimate new `dist/` module, which trains maintainers to edit the gate instead of reading it — so the count is reported and asserts nothing. The gate is chained into `npm run verify` (and therefore into `prepublishOnly`), runs as its own step in CI beside the three other packaging gates — on the Windows legs as well, which is what proves the script needs no `npm.cmd` shim — and is re-run in the release workflow. It is a local command first, never a hosted-only check. `src/__tests__/gate-surface.test.ts` pins it into the chain the same way it pins the other three packaging gates, by literal name as well as by the shared constant.

### Changed

- **The release workflow's npm upgrade is pinned to a major, and verified to have taken effect** (`.github/workflows/release.yml`). Trusted publishing needs npm >= 11.5.1 and the runner's Node 22 ships npm 10, so the workflow upgrades npm immediately before publishing. It did so with `npm@latest`, which puts an untested npm major on the critical path one step before `npm publish`: a major that dropped this workflow's Node line would break every release with no change on our side. It is now caret-bounded to `npm@^12` — the same rule the `overrides` block in `package.json` already follows — so the bump is deliberate rather than npm's release schedule. A check after the install asserts the 11.5.1 floor itself rather than the pinned major, so it keeps testing the property that matters and needs no edit when the pin moves; it catches a global install that resolved somewhere off `PATH`, which would otherwise surface as an opaque OIDC failure from `npm publish` after every gate had already passed.
- **The release-notes extractor now matches headings structurally, not textually** (`.github/workflows/release.yml`). Two problems in one `awk` pattern. The visible one: the alternative for this project's older `## DATE (vX.Y.Z - title)` headings listed a `\u2014` delimiter, and `awk` has no such escape, so it collapsed to the literal text `u2014` and matched nothing — dead, and redundant too, since the plain-space alternative already covered an em-dashed title. The one that actually bites: that legacy alternative was **unanchored**, so it matched its version string anywhere in any line rather than only in a heading. An ordinary prose mention of an old version — exactly what a changelog entry about the release tooling contains — therefore started a capture in the middle of the file and truncated that version's release notes to nothing. This was not hypothetical: writing the entry above reproduced it, taking `v1.3.5` from 27 lines to 1. Both alternatives are now anchored with `^## `, so a heading is required. Verified under `mawk` (the `awk` on the Ubuntu runners) against every version heading in this file: all 32 sections extract, none below 5 lines, with no bleed into the following section.
...
- **`check:tarball` could not read `npm pack --json` under npm 12, which blocked the v1.6.1 publish itself.** npm changed that output at its 12.0.0 major, silently: both forms are valid JSON carrying the same data, so a parser written against one simply finds no `files` array in the other.
  ...
  The failure mode is instructive. `release.yml` upgrades npm to `^12` immediately before publishing, and `npm publish` runs `prepublishOnly` (`npm run verify`), which ends with `check:tarball`. So the **same gate, on the same tree, in the same job** passed as the standalone "Verify tarball contents" step under the runner's npm 10, then failed minutes later under npm 12. It failed **closed** — refusing to report on a tarball it could not inspect — which is the correct direction, and is why nothing was published; but it blocked a release rather than catching a defect.

  Both shapes are now accepted: pinning only the new one would break every contributor still on npm 10 or 11 running `npm run verify` locally. The parsing was extracted into a side-effect-free `scripts/pack-listing.mjs` so it could be tested at all — it previously lived inline in a script that shells out to npm on import — and `src/__tests__/pack-listing.test.ts` pins both shapes, the notice-line recovery for each, and the fail-closed negatives (no `files` anywhere, a non-array `files`, truncated JSON, non-object input). Confirmed red before green: the previous parser returns `null` for the npm 12 shape.
- **The Argon2id defaults were misattributed to OWASP, in `README.md`, `SECURITY.md` and `src/core.ts`.** All three described the 128 MiB / `timeCost: 3` `HIGH` tier as OWASP's "first choice" for Argon2id, and the README additionally called `m=47104` its "preferred" configuration. Verified against the cheat sheet's raw source on 2026-09-10, the page does **two separate things** and conflating them is what produced the error: it **states a minimum** — "Use Argon2id with a minimum configuration of 19 MiB of memory, an iteration count of 2, and 1 degree of parallelism" — and it separately **lists five configurations** described as providing "an equal level of defense, and the only difference is a trade off between CPU and RAM usage" (`m=47104` (46 MiB) `t=1`, `m=19456` (19 MiB) `t=2`, `m=12288` (12 MiB) `t=3`, `m=9216` (9 MiB) `t=4`, `m=7168` (7 MiB) `t=5`, all at `p=1`). It designates none of them a "first choice": the ordering is a CPU/RAM trade-off, not a ranking. Corrected in the `SECURITY_THRESHOLDS` JSDoc (which ships in `dist/core.d.ts`), in `README.md` and in `SECURITY.md`, so the three now agree.
- **Security-relevant, and newly stated: the `MEDIUM` tier sits BELOW OWASP's stated Argon2id minimum.** `MEDIUM` is `memoryCost: 2 ** 14` (16 MiB) at `timeCost: 2` — the same iteration count OWASP asks for, with less memory than its 19 MiB floor. No document said so before; the internal reference had briefly claimed the opposite. `MEDIUM` is this library's own floor, not an OWASP-endorsed configuration, and the 12/9/7 MiB rows in OWASP's list are not cover for it, because each pairs its lower memory with *more* iterations. The Node default (`HIGH`, 128 MiB / `t=3`) and the browser default (32 MiB / `t=3`) both clear the minimum on both axes; only the `MEDIUM` *threshold* does not. **No default changed** — this is a correction to what the documentation claims about an existing threshold, not a change to any value.
- **Four JSDoc comments referenced a version that was never published** (`src/types.ts`, `src/utils.ts`). The `AES_256_CBC` removal and the `retryWithBackoff` default-policy change were both labelled "v0.19.0", a pre-1.0 development iteration that was rolled into 1.0.0; this changelog goes 0.18.2 → 1.0.0 and records both changes under 1.0.0. A consumer reading the shipped declarations could not look "v0.19.0" up on npm or here. All four now name **v1.0.0**, the version that actually shipped them. No behaviour is affected.
- **`codecov.yml` documented a coverage gate that does not exist.** Three comments stated that the local Jest threshold is "80% across branches/functions/lines/statements" and that Codecov's targets "match the local Jest floor". The real gate is the one-way ratchet in `jest.config.js` at statements 95, branches 87, functions 97, lines 95. The `target: 80%` values are unchanged and remain correct — they are a deliberately slack visualisation floor, and `if_ci_failed: success` means a Codecov status can never block a build — but the comments now say that instead of claiming a symmetry that was never there, and explicitly warn against "syncing" them to the Jest numbers.
- **`.npmignore` still instructed maintainers to treat any pack count other than 31 files as a failure**, which the `check:tarball` gate added in this release deliberately does not do (a count assertion goes red on every legitimate new `dist/` module). The caveat about npm force-including root `readme`/`license`/`licence`/`copying` files is retained, since that is the real hazard; the count instruction is replaced by a pointer to the gate.
- **`README.md`: a worked example printed output the code does not produce.** `createProgressBar(50, 100)` was shown with 16 filled and 14 empty blocks; it returns 15 and 15.
- **`README.md`: the `decryptContainer` metadata example contradicted itself.** The method reference printed `{ filename, mime, size }` while the usage example printed `{ size, filename, mime }`. `decryptContainer` builds `{ size }` first and appends `filename` then `mime`, so the second was right and both now match.
- **`README.md`: `decryptFileSync` was documented as writing with `fs.writeSync`**; it writes with `fs.writeFileSync(fd, chunk)`.
- **`README.md`: the `validatePath` traversal example asserted an error string that is Windows-only.** Both drive-letter accommodations are gated on `process.platform === 'win32'`, so on POSIX the same call is rejected earlier, with `'File path contains invalid characters'` rather than `'Path is outside the allowed root'`. Both outcomes are now shown.
- **`README.md`: a code example imported a type as a value.** `import { CryptoManager, ProgressCallback }` fails under `verbatimModuleSyntax` / `isolatedModules`; it is now a type-only import.
- **`README.md`: the `ARGON2_NOT_AVAILABLE` message was misquoted, and the internal reference rendered it wrongly.** The real message wraps `hash-wasm` in backticks. The README substituted straight quotes; the internal copy tried to escape the backticks inside a code span, which CommonMark does not support — the span closed early and the backslashes leaked into the rendered output. Both now use a fenced block and reproduce the string byte for byte.
- **`README.md`: three container error codes were missing** from a list that reads as exhaustive (`INVALID_CONTAINER_INPUT`, `CONTAINER_ENCRYPTION_FAILED`, `CONTAINER_DECRYPTION_FAILED`). All fourteen are now listed, grouped by when they can fire, with each one's `CryptoErrorType`. One correction came out of that: `INVALID_CONTAINER_INPUT` is **unreachable through the public API**, because `decryptContainer` type-checks its own argument and throws `INVALID_ENCRYPTED_DATA` before the parser that owns that code is ever called, and the parser is not re-exported from either entry point.
- **`README.md`: the browser secure-context failure was described as one uniform throw.** It is path-dependent: `crypto.subtle` is `undefined` on an insecure origin, so the key import raises a `TypeError` that propagates unchanged on the encrypt path but is converted to `CryptoError(DECRYPTION_FAILED)` on the decrypt path, which wraps the whole operation.
- **`README.md`: the WASM-versus-native Argon2id figure was an unsourced range.** "Roughly 2-3x slower" is replaced by a dated measurement — about 357 ms native against 631 ms WASM per derivation at the 128 MiB profile, native sampled ten times across 352-363 ms — together with a note that this repository records 395 ms for the same operation from an earlier session, so any absolute number should be treated as an order of magnitude.
- **`README.md`: incomplete public signatures.** `deriveKey`, `encryptData` and `decryptData` were documented without their optional trailing parameters (`overrides`, `aadOverride`), and the four file methods without `progress`. All are part of the public surface and are now documented with their semantics and their validation errors.

### Documented

- **The `legacyMode: 'auto'` error-masking behaviour, which was previously invisible.** In the default mode a header-parse failure is treated as a possible magic collision on a v0 salt, so `UNSUPPORTED_VERSION`, `UNSUPPORTED_KDF`, `KDF_MISMATCH`, `INVALID_HEADER_PARAM` and `TRUNCATED_HEADER` are all swallowed and the blob is retried as v0. `KDF_PARAMS_OUT_OF_BOUNDS` is the sole code re-thrown in every mode, so DoS rejection is never traded away for legacy recovery. What a caller sees instead depends on length: under `salt + iv + tag` = 60 bytes the retry fails its size check and reports `INVALID_ENCRYPTED_DATA_SIZE`; at 60 bytes or more it reaches GCM and reports the generic `DECRYPTION_FAILED`. Anyone diagnosing a format problem should construct a `strict` manager to see the real code. Behaviour is unchanged; only the documentation of it is new.
...
Performance, browser type safety, and a cryptographic bound that was missing. v1.5.0's isomorphic refactor re-expressed Node's async text path over the pure `Uint8Array` codec, which cost it 70-140x against the `Buffer` built-ins it used before; that regression is gone, and the pure codec every non-Node consumer runs got 2.0-11.4x faster on encode and 3.4-4.6x on decode in its own right. `inspectHeader` no longer decodes a whole ciphertext to read 22 bytes. A Node type that had leaked into the browser declaration graph is out, and a new compile gate keeps it out. Payloads past the AES-GCM per-invocation limit are refused instead of silently mis-encrypted. **Not one output byte moved**: the wire format is byte-identical to 1.5.0 on every path, and no primitive, key/salt/output size or KDF default changed. Two changes deserve a second look before upgrading: the new `DATA_TOO_LARGE_FOR_GCM` refusal (under Fixed) and the removal of `EncryptionResult` from the **browser** type surface (under Changed); the Node surface loses nothing and gains three exports (`MAX_GCM_PLAINTEXT_BYTES`, `assertGcmPlaintextLimit`, `CONTAINER_VERSION`) plus a second import path for `EncryptionResult`.
...
- **Browser declaration-purity gate** (`scripts/check-browser-types.mjs`, `package.json`, `.github/workflows/ci.yml`). New `npm run check:types:browser` type-checks a throwaway browser-only consumer — resolved through the package's `browser` export condition, with `"types": []` and `"skipLibCheck": false` — against the built `dist/`, so any Node-only type that reaches `dist/index.browser.d.ts` or anything it re-exports fails the build. It runs in CI beside `check:browser` and is chained into `prepublishOnly`. Neither pre-existing static gate can see this defect class: `check:browser` bundles with esbuild, which erases types, and the ESLint `no-restricted-globals` override does not fire on a type-position `Buffer`.
- **The v2 container version is now nameable from the public API** (`src/core.ts`, `src/crypto-manager.ts`, `src/crypto-manager.browser.ts`, `src/index.ts`, `src/index.browser.ts`). `CONTAINER_VERSION` (`0x02`) was defined in the isomorphic core but reached no entry point, so a consumer could not name the version of a format this library produces — container mode lives on the shared core, so both runtimes emit v2 blobs. It is now re-exported from the Node and the browser entry alike, symmetrically with `FORMAT_VERSION` (`0x01`), which makes routing a blob a one-line check on the byte at offset 4 without decrypting it. Nothing else changed: no wire byte moved, no existing export was renamed, and the constant's value and definition site are untouched. `README.md` gains a "Telling the formats apart" subsection with the classification snippet, including why the magic bytes must be checked before the version byte (a v0 ciphertext's random salt begins with `HPCR` roughly once in `2 ** 32`) and why that snippet compares `MAGIC_BYTES` directly rather than calling the exported `hasMagic()`, which on the Node entry keeps its original contract of accepting only a real `Buffer` and so returns `false` for the `Uint8Array` that `encryptBytes` / `encryptContainer` return. The same pass documents a previously unstated `inspectHeader` behavior in both `README.md` and its JSDoc: it is a v1 inspector, not a format sniffer, so a v2 container — which carries the same `HPCR` magic — makes it **throw** `CryptoError(DECRYPTION_FAILED, 'UNSUPPORTED_VERSION')` rather than return `null`. That behavior is unchanged; only the documentation was missing.
- **One command runs every gate: `npm run verify`** (`package.json`, `jest.config.js`). It chains `lint`, `type-check`, `build`, `test`, `check:browser`, `check:types:browser` and `check:exports`, stopping at the first failure, and `prepublishOnly` is now defined as exactly `npm run verify` so the release gate and the gate a working session runs cannot drift apart. The composed chain is identical to what `prepublishOnly` already ran, so nothing was added to or removed from the publish gauntlet. Alongside it, the Jest `coverageThreshold.global` floor was ratcheted from 80/80/80/80 to the measured values floored to integers (statements 95, branches 87, functions 97, lines 95); the thresholds move up only, and no `collectCoverageFrom` exclusion was added to reach them. The three gate tiers (FAST, FULL, BROWSER), their exact commands and their measured wall-clock durations are tabulated in `README.md` under Testing, and a new `src/__tests__/gate-surface.test.ts` pins the gate surface itself: it fails if a gate leaves the `verify` chain, if `prepublishOnly` stops delegating to it, if the chain gains a `;` or `||` that would let a failing gate pass, or if a coverage threshold is lowered or a `collectCoverageFrom` exclusion added.
- **A KDF-free codec benchmark** (`bench/codec.mjs`, `bench/index.mjs`, `bench/README.md`). `npm run bench` now runs a codec group first, 14 rows in total: `bytesToBase64url` and `base64urlToBytes` at 1 KiB, 1 MiB and 8 MiB, each paired with a reference row running the exact `Buffer` expression the Node build overrides the seam with (`Buffer.from(b.buffer, b.byteOffset, b.byteLength).toString('base64url')` and `Buffer.from(s, 'base64url')`, not the copying `Buffer.from(bytes)`), plus `inspectHeader` on a 1 MiB-plaintext ciphertext in both of its input forms — the base64url string, and a `Uint8Array` as the reference. So the gap between the pure codec and the native path is visible rather than inferred. **No measured case performs a key derivation**, which is the entire point: one Argon2id derivation at the default 128 MiB profile costs ~100-400 ms depending on the host and would hide any codec regression completely inside the other benchmark files. The group runs in ~30 s, and `bench/` stays out of the published tarball (`npm pack --dry-run`: 31 files, none of them under `bench/`).

### Changed

- **The base64url codec is much faster on both the Node and the pure path, and still byte-identical** (`src/codec.ts`, `src/core.ts`, `src/crypto-manager.ts`). Two changes, neither of which forks the logic. First, the pure codec was rewritten: a single pass over the input with a typed-array scratch buffer instead of a growing `number[]`, chunked `String.fromCharCode.apply` at 8192 output characters, and an O(n) allocation-free canonical check in `isValidBase64url` that replaces a decode-then-re-encode-then-string-compare round trip. Second, `CryptoCore` grew a protected `encodeBase64url` / `decodeBase64url` seam that defaults to that pure codec, and the Node `CryptoManager` overrides it with `Buffer.from(bytes.buffer, bytes.byteOffset, bytes.byteLength).toString('base64url')` and `Buffer.from(s, 'base64url')` — so Node uses its native implementation again without the browser losing the pure one. Measured on Node v24.19.0: the pure codec is **2.0x / 3.2x / 11.4x** faster on encode and **3.4x / 3.7x / 4.6x** on decode at 64 KiB / 1 MiB / 8 MiB, and `isValidBase64url` is **15-30x** faster over the same range. On the Node path, the isolated base64url step of a 4 MiB payload fell from **49.3 ms to 3.1 ms** on encode and from **49.7 ms to 3.8 ms** on decode, taking end-to-end 4 MiB `decryptText` from **179 ms to 42 ms** with an identical key-derivation profile. (End-to-end `encryptText` at that size is dominated by AES-GCM and UTF-8 encoding, so the codec win is real but not what the wall clock there reports.) The output contract is unchanged in both directions, including the decoder's exact leniency rules — line-wrapped input, the standard `+`/`/` alphabet, `=` terminating, any non-alphabet byte skipped rather than fatal, a trailing incomplete sextet discarded. Equivalence was proven differentially against the previous implementation and against Node's `Buffer` over 2 697 181 cases, including exhaustively over all 1 118 481 strings of length 0-5 drawn from an adversarial alphabet.
- **`inspectHeader` no longer decodes a whole ciphertext to read 22 bytes** (`src/core.ts`). For a base64url string it now decodes only the first 32 characters: `ceil(22 * 4 / 3) = 30` characters carry the header, and rounding up to the next multiple of 4 lands on a base64 quantum boundary, so the slice of a canonical string is itself canonical and decodes to 24 bytes — a byte-exact prefix of the full decode, with two bytes to spare. Every contract is unchanged, deliberately: the canonical-form check still runs over the **whole** string (narrowing that would stop a malformed tail from being rejected), a malformed string still throws `CryptoError(INVALID_INPUT, 'INVALID_BASE64URL')`, a v0 ciphertext still returns `null`, a v2 container still throws `UNSUPPORTED_VERSION`, and byte inputs are still read as-is. Together with the codec rewrite this takes a 4 MiB ciphertext (5 592 515 characters) from **697 ms to 18.55 ms**, roughly **37x** — but the number that matters is the allocation: **~4 194 386 bytes of garbage per call became 24 bytes**, because the full-length decode was the last allocation left on the path. What remains is the mandatory whole-string canonical scan, which is now O(n) and allocates nothing.
- **`EncryptionResult` moved from `src/types.ts` to `src/crypto-manager.ts`, and is no longer part of the browser type surface.** The interface names the Node `Buffer` global, and `types.ts` is re-exported by the browser entry, so a browser-only TypeScript project (no `@types/node`, `"types": []`, `"skipLibCheck": false`) got two hard `TS2591 Cannot find name 'Buffer'` errors compiling against 1.5.0. **Node consumers are unaffected, and the Node surface strictly grows**: `import type { EncryptionResult } from '@hiprax/crypto'` — the only path that carried the type before — keeps working unchanged, and the type is now additionally exported from `@hiprax/crypto/crypto-manager`, which it was not reachable from in 1.5.0 (that subpath's declarations only type-imported the name and never re-exported it). Those two are the only paths in the `exports` map that expose it. Browser-condition consumers lose the name — deliberately: nothing on the browser surface can produce one (`encryptData` there throws `UNSUPPORTED_IN_BROWSER` and returns a local `Uint8Array`-shaped type), and the type referenced a global that does not exist in a browser. A browser-condition consumer that imported the name should drop the import; there is no browser replacement because there is no browser producer.
...
- **Oversized payloads are no longer silently mis-encrypted; they are refused** (`src/core.ts`, `src/crypto-manager.ts`, `src/format-core.ts`). **This is a deliberate behavior change, and it is the fix rather than a regression.** `encryptBytes`, `decryptBytes`, `encryptData`, `decryptData`, `encryptFile`, `encryptFileSync`, `decryptFile` and `decryptFileSync` now throw `CryptoError(INVALID_INPUT, 'DATA_TOO_LARGE_FOR_GCM')` for anything larger than `MAX_GCM_PLAINTEXT_BYTES` (`2 ** 36 - 32` = 68 719 476 704 bytes, ~64 GiB). Those calls previously *succeeded* — and that was the defect: NIST SP 800-38D section 5.2.1.1 caps one AES-GCM invocation at `2 ** 39 - 256` bits, and past that the 32-bit block counter wraps, the keystream repeats, and the ciphertext produced has **neither confidentiality nor authenticity**. Neither OpenSSL's EVP layer nor Node's `crypto.createCipheriv` enforces the bound, so the previous behavior was to hand back a broken artifact with no error. There is deliberately **no opt-out**: an opt-out would be an opt-in to a broken construction. The check runs before any key derivation, before any temp file is created and before any cipher is constructed, so a refused call writes no `.tmp` file, writes no output file and wastes no key derivation. (The two decrypt file paths still create a missing output *directory* before the check, as they always have; that ordering is unchanged.) Payloads at exactly the limit are still accepted, and no *sound* ciphertext is affected — reaching the bound takes a single ~64 GiB call, and v2 containers were already capped at just under 4 GiB (`0xffffffff` bytes, an internal limit reported as `CONTAINER_DATA_TOO_LARGE`), far below the GCM bound. **One upgrade note, because the bound is asserted on the ciphertext body as well as on the plaintext:** a ciphertext larger than the bound that was produced by 1.5.0 or earlier is now *refused on decryption* rather than decrypted. Counter wrap is deterministic and the tag mask is fixed, so an earlier version could round-trip its own oversized output; that output never had confidentiality or authenticity, but it was readable, and this release stops reading it. That is the intended outcome, not an oversight — but if you hold such an artifact (a single file over 68,719,476,704 bytes, plausible for a disk image or backup), pin `@hiprax/crypto@1.5.0` to recover the plaintext, then re-encrypt it in chunks under 1.6.0.
- **The async file paths now open their input exactly once** (`src/crypto-manager.ts`). `encryptFile` and `decryptFile` previously resolved `inputPath` three times per call — an `existsSync` pre-check, a `statSync` (encrypt) or `open` (decrypt), and then a separate `createReadStream(inputPath)` for the body — a time-of-check/time-of-use window in which the bytes that got encrypted, or the body authenticated against a header read earlier, need not have come from the file that was checked. Both now open the path once via `fs/promises.open`, take the size from that handle's own `stat()`, stream the body from the same descriptor (`{ start: 0 }` for encryption, `{ start, end }` for the bounded ciphertext body), and close it unconditionally in a `finally` that every early return path reaches. A missing input still reports `CryptoError(FILE_ERROR, 'INPUT_FILE_NOT_FOUND')` with the same message, and a failure still leaves no `.tmp` file and no output file behind.
- **An Argon2 provider that loads but exposes no callable hasher is now refused, so the WASM fallback still gets its turn** (`src/engine.node.ts`). `importNativeArgon2` chose the imported namespace's `.default` on bare truthiness, with no check that the object it picked could actually hash. For a module shaped `{ hash, default: {} }` it therefore resolved the empty object, tagged it `provider: 'native'`, and the module-level loader cached that resolved promise for the lifetime of the process — after which every async call failed with `CryptoError(ENCRYPTION_FAILED, 'KEY_DERIVATION_FAILED')` carrying an internal variable name in its message, and `hash-wasm` was never attempted even when installed and working. `importHashWasmArgon2` had the same gap one step later: it checked `.default.argon2id` for callability but then fell back to the namespace unconditionally, so a module with no callable `argon2id` anywhere produced the same misleading call-time failure — where the browser engine reports the correct `MEMORY_ERROR` / `ARGON2_NOT_AVAILABLE` for the identical shape. Both loaders now select a candidate only when its entry point is callable and throw otherwise, which lets the existing native-then-WASM chain fall through and, when both providers are unusable, name **both** causes in one `ARGON2_NOT_AVAILABLE`. `.default` is still preferred over the namespace when both are callable, so behavior on the real, CJS-published `argon2` package is unchanged; no released version of either package produces the broken shape, so this closes a robustness gap in the branch that exists to cope with non-standard module shapes rather than a live failure.

### Files changed

- `src/codec.ts` — `bytesToBase64url` / `base64urlToBytes` rewritten (single pass, typed-array scratch, chunked `String.fromCharCode.apply` at a module-private `B64_ENCODE_CHUNK = 8192`); `isValidBase64url` replaced by an O(n) allocation-free canonical-form check. Public names, signatures and outputs unchanged.
- `src/core.ts` — new protected `encodeBase64url` / `decodeBase64url` seam (defaulting to the pure codec) used by `encryptText` / `decryptText` / `inspectHeader`; `inspectHeader` decodes only `HEADER_B64URL_PREFIX_CHARS` (32) characters of a string input; `encryptBytes` / `decryptBytes` assert the AES-GCM bound; `CONTAINER_VERSION` passed through to the entry points; the existing `MAX_CONTAINER_DATA_SIZE` cap documented as already sitting below the GCM bound.
- `src/crypto-manager.ts` — Node `Buffer` overrides of the base64url seam; the AES-GCM bound asserted in `encryptData`, `decryptData` and all four file methods; `encryptFile` / `decryptFile` open their input exactly once via `fs/promises.open` and stream from that descriptor; `decryptFileSync` derives its body range before key derivation; `EncryptionResult` now lives and is exported here.
- `src/format-core.ts` — new `MAX_GCM_PLAINTEXT_BYTES` and `assertGcmPlaintextLimit(byteLength, code?)`.
- `src/format.ts` — both new symbols added to its named re-export list, so the Node and browser surfaces stay symmetric; `hasMagic`'s JSDoc now records that it accepts only a real `Buffer` and therefore returns `false` for the plain `Uint8Array` that `encryptBytes` / `encryptContainer` return, pointing at the `MAGIC_BYTES` comparison that behaves identically on both entry points.
- `src/engine.node.ts` — both Argon2 provider loaders refuse a module that loads without a callable hasher (see Fixed).
- `src/types.ts` — `EncryptionResult` removed (moved to `crypto-manager.ts`), with a comment recording why the module must stay Node-free.
- `src/index.ts` — re-exports `CONTAINER_VERSION` and `type EncryptionResult` (the latter as its own `export type` statement, which `verbatimModuleSyntax` requires).
- `src/index.browser.ts`, `src/crypto-manager.browser.ts` — re-export `CONTAINER_VERSION`.
- `src/__tests__/` — new `codec-seam.test.ts`, `inspect-prefix.test.ts`, `gcm-limit.test.ts`, `engine-web-interop.test.ts` and `gate-surface.test.ts`; `codec.test.ts`, `esm-smoke.test.ts`, `interop.test.ts`, `streaming-cleanup.test.ts`, `engine-web.test.ts` and `argon2-lazy-load.test.ts` extended; `browser/interop.browser.test.ts` gained v2-container specs. The Jest suite stands at 25 suites / 1113 tests / 25 snapshots, up from 20 suites / 948 tests at 1.5.0.
- `src/__tests__/fixtures/node-vectors.json` — new `containers` array of Node-produced v2 container goldens the browser build decrypts. The six pre-existing v1 `vectors` were regenerated in the same run, so they carry fresh salts and IVs and are no longer the literal bytes 1.5.0 shipped; the format is unchanged, and the released 1.5.0 vectors were re-checked against this build and still decrypt.
- `scripts/check-browser-types.mjs` — new browser declaration-purity gate.
- `scripts/gen-node-vectors.mjs` — also emits the self-verified v2 container goldens.
- `jest.config.js` — `coverageThreshold.global` ratcheted from 80/80/80/80 to the measured statements 95 / branches 87 / functions 97 / lines 95.
- `eslint.config.ts` — `src/types.ts` added to the isomorphic browser-graph override; the override's scope limit (it cannot see a type-position `Buffer`) documented in place, pointing at the compile gate that can.
- `package.json` — new `check:types:browser` and `verify` scripts; `prepublishOnly` redefined as exactly `npm run verify`; `version` bumped `1.5.0` → `1.6.0`.
- `package-lock.json` — `version` synced to `1.6.0`.
- `.github/workflows/ci.yml` — new "Check browser declaration purity (no Node types)" step beside `check:browser`; the Codecov comment corrected to name the ratcheted thresholds.
- `bench/codec.mjs` — new KDF-free codec + `inspectHeader` benchmark group; `bench/index.mjs` runs it first, `bench/README.md` documents it, `bench/kdf.mjs` header comment corrected to its measured runtime.
- `README.md` — the three gate tiers tabulated with their measured durations, the three static browser gates and `npm run verify` documented, `DATA_TOO_LARGE_FOR_GCM` and the GCM bound added to the error-handling section, `CONTAINER_VERSION` / "Telling the formats apart" added to the format documentation, `inspectHeader` documented as a v1 inspector that throws on a container, `EncryptionResult` noted as Node-only.
- `.npmignore` — a comment recording that `*.tmp` cannot exclude a root file whose name before the first dot is `README`: npm force-includes those, overriding both this file and the `files` allowlist, so a scratch `README.md.tmp` would be published. Measured by probe; no configuration suppresses it, so the rule is to delete such a file before packing.
- `README.md`, `SECURITY.md` — factual corrections in both: the header version byte `0x02` was still described as an unallocated escape hatch for a future cipher change, but it has been the v2 container version since 1.5.0 (as `SECURITY.md` already said elsewhere in the same file). `0x03` onward are the free values. No security-posture, primitive, size or KDF statement changed. `SECURITY.md` additionally now describes **three** browser-isolation gates rather than two: its threat-model note predated `check:types:browser` and so credited the isolation guarantee to the two gates that structurally cannot see a **type-position** Node reference — precisely the defect class this release fixes and that v1.5.0 shipped. `README.md` already documented all three.
...
- **Browser build + conditional exports** (`src/crypto-manager.browser.ts`, `src/index.browser.ts`, `package.json`). The `.` export is now conditional (`browser → node → default`, each branch nesting its own `types` + `default`): browser bundlers (webpack, Vite/Rollup, esbuild, Next.js) resolve a separate `dist/index.browser.js` whose import graph contains **zero `node:` builtins** and references no `Buffer`/`process` global, while Node resolves `dist/index.js` unchanged. Because the two builds have genuinely different public surfaces (the Node build re-exports the `./utils` file helpers and the `Buffer`-typed `./format`; the browser build omits `utils` and re-exports the `Uint8Array`-typed `./format-core`), each condition nests its **own** declaration file (`dist/index.browser.d.ts` for `browser`, `dist/index.d.ts` for `node`/`default`) so a browser consumer never resolves the Node-only, `Buffer`-typed declarations. The browser `CryptoManager` uses Web Crypto (SubtleCrypto) for AES-256-GCM/SHA-256/CSPRNG and WebAssembly `hash-wasm` for Argon2id, with a lighter **32 MiB Argon2id default** (`memoryCost = 2 ** 15`, classified `MEDIUM`) to avoid OOM on memory-constrained mobile browsers. Node-only methods (sync/PBKDF2, streaming file, `Buffer`-typed low-level) are throwing stubs that raise `CryptoError(INVALID_INPUT, 'UNSUPPORTED_IN_BROWSER')`. The browser build does not export the Node-only `./utils` file helpers.
...
- `CHANGELOG.md` — this entry (also promotes the previously-unreleased OIDC trusted-publishing workflow note into this release); historical entries scrubbed of references to gitignored internal working files, which never ship and should not be cited by the public changelog.
- `src/__tests__/crypto-manager.test.ts` — one comment's documented-contract pointer updated to cite only public docs (comment-only; no behavior change).
- `.github/PULL_REQUEST_TEMPLATE.md` — the docs checklist item now names only `README.md`; it previously also asked contributors to update a gitignored internal file that does not exist in checkouts of this repo.
- `.gitignore` / `.npmignore` — section comments above the local-working-file ignore patterns reworded.
...
- **Documentation synchronised with the real `CryptoManager` API surface** (`README.md`). The constructor Options list was missing `skipPasswordValidation` and `legacyHeaderAad` (both were already described in prose elsewhere); both are now listed with cross-links to the Password Requirements and v1.0.0 → v1.1.0 Migration sections. The Methods section gained entries for `getLegacyMode(): LegacyMode` and `inspectHeader(input: string | Buffer): ParsedHeader | null` (documenting the `null`-for-v0 return and the `INVALID_BASE64URL` fail-fast for malformed strings, cross-linking the existing worked example rather than duplicating it). Docs-only; no code or behavior change.
...
- **Real cross-provider Argon2id known-answer parity tests** (`src/__tests__/argon2-provider-parity.test.ts`). New file with no module mocks: it runs the genuine locally-installed native `argon2` and pure-WASM `hash-wasm` KDFs and asserts both derive an identical pinned 32-byte known-answer vector (computed live at implementation time from argon2@0.44.0 and hash-wasm@4.12.0; never authored from memory). Each test uses a "probe is the call" availability gate — the real derivation runs inside a try/catch and performs a logged early-return skip if the provider is genuinely unavailable (a bare `import('argon2')` is insufficient because a broken native binding still imports), so a missing compiler skips rather than fails.
...
- **`createProgressBar` no longer throws `RangeError` or emits `NaN%` on out-of-range input** (`src/utils.ts`). Previously, only `total` was guarded (`total <= 0` early return), and the percentage was clamped at the top end only (`Math.min(current / total, 1)`). Consequences: a negative `current` (e.g. `-5`) produced `percentage < 0`, so `Math.round(width * percentage)` was negative and `'█'.repeat(negative)` threw `RangeError: Invalid count value`; a negative `width` (e.g. `-3`) triggered the same on the `empty` side; a non-finite `current` (NaN, Infinity) silently produced malformed output such as `'[] NaN%'`. Reproduced live against `dist/utils.js`: `createProgressBar(-5, 10)` → `RangeError: Invalid count value: -15`; `createProgressBar(5, 10, -3)` → `RangeError: Invalid count value: -1`; `createProgressBar(NaN, 10)` → `'[] NaN%'`. The fix: (a) clamps the percentage at **both** ends via `Math.min(Math.max(current / total, 0), 1)` and treats any non-finite `current` or `total` as `0` progress via `Number.isFinite()` checks; (b) sanitizes `width` to a positive integer, falling back to the documented default `30` when it is not (using `Number.isInteger(width) && width > 0`). The validated `w` variable is used for both `filled` and `empty` repeat calls so neither can be negative. The existing `total <= 0` early return and its exact 30-character empty-bar string are unchanged. The helper remains a pure display utility — it never throws; garbage input produces a sensible zero-or-full bar. New tests (`src/__tests__/utils.test.ts`): six new edge-case assertions in `'createProgressBar - robustness (Phase 1 hardening)'` covering negative current, negative width, NaN current, Infinity current, non-integer width, and zero width — each asserting a well-formed `/^\[.*\] \d+%$/` string without throwing; plus a regression block confirming the existing happy-path, over-cap, and `total<=0` outputs are unchanged.
- **`sha256`, `getFileExtension`, and `isTextFile` now throw typed `CryptoError` for non-string input** (`src/utils.ts`). Previously, all three passed non-string values directly to the underlying Node.js API (`crypto.createHash().update()` and `path.extname()`), which threw a raw `TypeError (ERR_INVALID_ARG_TYPE)` — inconsistent with the library's "all errors are `CryptoError`" principle and with their guarded siblings (`isValidBase64`, `isValidBase64Url`, `secureStringCompare` all check `typeof` and return a safe value or typed error). Two independent judges were summoned to resolve the split-verdict from the original audit; both unanimously chose **Option A** (typed `CryptoError` guard), reasoning that: (1) the "all errors are CryptoError" contract is explicit in the codebase and the v1.4.1 `aad` guard set the same precedent; (2) `sha256` has no safe hash value for non-string input — hashing a coerced `''` would silently produce a wrong answer; (3) throwing a typed error is more informative for callers who already catch `CryptoError`. The fix adds a `typeof input !== 'string'` guard to each function throwing `CryptoError(INVALID_INPUT, 'INVALID_INPUT')` with a descriptive message. Backward compatible for valid usage: string inputs are unaffected; non-string inputs previously crashed with a native `TypeError` and now throw a typed `CryptoError`. New tests (`src/__tests__/utils.test.ts`): three new `it` blocks — one each in `'getFileExtension'`, `'isTextFile'`, and `'sha256 - additional cases'` — each iterating `[123, null, undefined, {}, []]` and asserting `CryptoError` with `code: 'INVALID_INPUT'` and `type: INVALID_INPUT`.
- **Plaintext buffers scrubbed on error paths in all four text encryption/decryption methods** (`src/crypto-manager.ts`). `encryptText` and `encryptTextSync` declared their plaintext buffer (`textBuffer = Buffer.from(text, 'utf8')`) as `const` inside the `try` block, and `decryptText` and `decryptTextSync` declared their decrypted output (`decrypted = this.decryptData(...)`) the same way. As a result, the `catch` block in each method — which already scrubbed the derived key — could not reference these buffers at all. If anything threw after the buffer was allocated (for the encrypt paths: a failure inside `encryptData`; for the decrypt paths: any exception in the narrow window after `decryptData` returns), up to a full message worth of plaintext could linger un-zeroed in heap until GC. This is the exact asymmetry the v1.4.1 release already closed for `encryptFileSync`'s 64 KiB plaintext chunk. The fix applies the same pattern to all four text methods: hoist each buffer to `let … = null` before the `try`, assign at the existing creation site, keep the success-path `secureClear` calls unchanged, and add `if (buffer !== null) { this.secureClear(buffer); }` at the top of each `catch` block alongside the existing key scrub. The `combined` subarray scrub on the decrypt success path is untouched. No change to wire format, public API, or existing behavior — this is defense-in-depth memory hygiene. New tests (`src/__tests__/crypto-manager.test.ts`, describe `'Phase 2 — text-method plaintext-buffer scrub on error path'`): (a) for each encrypt path, spy on `encryptData` to throw after `textBuffer` allocation and assert `secureClear` was called with the plaintext buffer via a length-discriminated `clearSpy.mock.calls.find`; (b) for each decrypt path, assert the round-trip and wrong-password behaviors are unaffected by the hoist.

- **`parseHeader` now enforces the Argon2id RFC 9106 §3.1 cross-field floor `memoryCost >= 8 * parallelism`** (`src/format.ts`). `parseHeader`'s Argon2id branch validated each parameter independently (positive, under the per-field DoS caps) but not the cross-field relationship Argon2id itself requires. A crafted v1 header with e.g. `memoryCost=8, parallelism=64` (both positive, both under their respective caps, but `8 < 8×64=512`) passed `parseHeader` and `assertKdfMatches`, then reached `deriveKey` where `argon2.hash` rejected it — surfacing as `CryptoError(type: ENCRYPTION_FAILED, code: KEY_DERIVATION_FAILED)`, an encryption-typed error on a decryption operation. Reproduced live against `dist/`: `decryptText(craftedBase64url, pwd)` → `ENCRYPTION_FAILED / KEY_DERIVATION_FAILED` / "Key derivation failed: Memory cost is too small". The `CryptoManager` constructor has enforced the same floor since v1.4.1 (code `MEMORY_COST_TOO_SMALL`), so no legitimately-produced ciphertext can carry sub-floor params — the new check can only fire on attacker-crafted input. Two independent read-only judges confirmed the design (both APPROVE, HIGH confidence): use `CryptoErrorType.DECRYPTION_FAILED / 'INVALID_HEADER_PARAM'` (same type/code as the existing zero/negative-param check immediately above), not `INVALID_INPUT / KDF_PARAMS_OUT_OF_BOUNDS` (reserved for DoS-scale upper-bound violations). Using `DECRYPTION_FAILED / INVALID_HEADER_PARAM` means `legacyMode: 'auto'` falls through to the v0 recovery path identically to other malformed-header cases (one KDF call at the instance's default parameters, then GCM auth rejects the result with `DECRYPTION_FAILED`); `strict` and `reject` modes re-throw immediately with `DECRYPTION_FAILED / INVALID_HEADER_PARAM`. No path can now produce `ENCRYPTION_FAILED` from a crafted sub-floor header. `packHeader` is intentionally unchanged — it is a low-level serializer called only from the library's own encryption paths, which always go through the constructor floor first.
- **README "CommonJS interop" section rewrote to match Node 22+ reality** (`README.md`). The section previously claimed that omitting the `require` export key causes CommonJS consumers to receive "a clear `ERR_REQUIRE_ESM`". On Node 22+ (the package minimum) a `require()` call against `dist/index.js` — which has no top-level `await` — actually **succeeds** via Node's built-in `require(esm)` path, returning the full namespace without error. Verified live: `node -e "require('./dist/index.js')"` → `{ CryptoManager: [Function], … }` on Node 24. The false hard-failure guarantee was removed; the section now documents that on Node 22+ `require()` may succeed, that this is untested by the library, and that `await import()` from an `async` function remains the supported and portable interop path. The code example is unchanged.
- **Stale progress-event comment in async `decryptFile` corrected** (`src/crypto-manager.ts`). The comment immediately before the initial `invokeProgress(progress, 0, totalSize)` call in the async `decryptFile` path described the first event as reporting "the bytes already read out of the file (header + salt + iv + tag)" and used "processed" semantics — but the actual call passes `0` as the first argument, not `consumedFrontAndTag`. The `decryptFileSync` sibling had the correct comment ("`(0, totalSize)` start sentinel") since it was written. The async comment is now updated to match: the first event is a deterministic start sentinel of `(0, totalSize)` that lets callers distinguish "stream started" from "first chunk processed"; subsequent events report cumulative `consumedFrontAndTag + bodyConsumed` bytes. Code is unchanged; this is a comment-only fix.
- **Two JSDoc blocks corrected from "prior to 0.10.0" to "prior to 0.11.0"** (`src/types.ts`, `src/crypto-manager.ts`). The `legacyPbkdf2Iterations` JSDoc in `src/types.ts` and the `PBKDF2_LEGACY_ITERATIONS` constant JSDoc in `src/crypto-manager.ts` both described the legacy v0 format as produced by "versions of this library prior to 0.10.0". The canonical CHANGELOG and README both record the version cutover as 0.11.0 (the release that introduced the versioned HPCR header). Source JSDoc was the lone outlier. Updated in both files to "prior to 0.11.0". Runtime is unchanged; this is a doc-only correction.
...
- `src/__tests__/utils.test.ts` — new describe block `'createProgressBar - robustness (Phase 1 hardening)'` (6 edge-case tests + 1 regression block); new `it` in `'getFileExtension'` for non-string guard; new `it` in `'isTextFile'` for non-string guard; new `it` in `'sha256 - additional cases'` for non-string guard.
- `src/crypto-manager.ts` — `encryptText`: hoist `textBuffer` to `let … = null` before `try`; change `const textBuffer =` to plain assignment; add null-checked `secureClear(textBuffer)` in `catch`. `encryptTextSync`: same. `decryptText`: hoist `decrypted` to `let … = null` before `try`; change `const decrypted =` to plain assignment; add null-checked `secureClear(decrypted)` in `catch`. `decryptTextSync`: same. `decryptFile` (async): replace stale progress-event comment with accurate `(0, totalSize)` start-sentinel description matching `decryptFileSync`. `PBKDF2_LEGACY_ITERATIONS` constant JSDoc: "prior to 0.10.0" → "prior to 0.11.0".
- `src/__tests__/crypto-manager.test.ts` — new describe block `'Phase 2 — text-method plaintext-buffer scrub on error path'` (4 tests: encrypt error-path scrub × 2, decrypt regression × 2).
- `src/format.ts` — `parseHeader` Argon2id branch: add RFC 9106 §3.1 floor check after the DoS-cap check; update JSDoc to document the floor and its `DECRYPTION_FAILED / INVALID_HEADER_PARAM` choice.
- `src/__tests__/format-fuzz.test.ts` — `assertWellFormedParseResult`: add `memoryCost >= 8 * parallelism` floor assertion in the Argon2id branch to lock in the new parse invariant.
...
- `README.md` — "CommonJS interop" section: remove false `ERR_REQUIRE_ESM` claim; document Node 22+ `require(esm)` success. `createBackupPath` code example: show real output shape including `_<6 hex>` random discriminator.
...
- `src/crypto-manager.ts` (`encryptFileSync`, `decryptFileSync`), `src/__tests__/streaming-cleanup.test.ts` — eliminate two "Potential file system race condition" (TOCTOU) alerts. In `encryptFileSync` and `decryptFileSync`, replaced the `existsSync(inputPath)` pre-check + later `openSync(inputPath)` pattern with opening the fd first (ENOENT re-thrown as `INPUT_FILE_NOT_FOUND`; other errors propagate); replaced `statSync(inputPath).size` with `fstatSync(inputFd as number).size` so there is no second path-based lookup after the open. In the test, replaced `existsSync(decPath)` + `statSync(decPath).size` + `readFileSync(decPath)` with a single `readFileSync(decPath)` call (existence is implicitly verified by the read).
...
- **`decryptFile` async: host-process crash eliminated on empty-body ciphertext with a failing output stream** (`src/crypto-manager.ts`). The async `decryptFile` path created `outputStream` but attached no `'error'` listener to it. For an empty-body ciphertext (produced from a 0-byte plaintext file) where `bodyLen === 0`, the body pipeline is skipped entirely; if the temp-file open failed (e.g. the output directory was actually a regular file → `ENOTDIR`) the stream emitted `'error'` with no listener, Node raised `ERR_UNHANDLED_ERROR`, and the host process crashed. By contrast, the non-empty-body path routes through `pipeline()` (which attaches its own error listener) and was safe; all sync paths use file descriptors, not streams. The fix attaches a persistent `onStreamError` listener immediately after `createWriteStream`, mirroring the identical guard that `encryptFile` has had since v1.4.0. New test (`src/__tests__/streaming-cleanup.test.ts`): mock `createWriteStream` returning a `Writable` that destroys itself on `process.nextTick` (simulating a failed open); `decryptFile` on a valid empty-body ciphertext must now **reject with `CryptoError`** (not crash the process), leave no orphan `.tmp`, and leave any pre-existing output path untouched.
- **`encryptFile` and `decryptFile` async: real stream errors no longer masked by `ERR_STREAM_DESTROYED`** (`src/crypto-manager.ts`). The `finally` block in both methods called `await new Promise((resolve, reject) => outputStream.end(err => err ? reject(err) : resolve()))`. When the destination stream had already been destroyed (e.g. by `pipeline()` after an `ENOSPC` error mid-body), `end(cb)` called `cb` with `ERR_STREAM_DESTROYED`; because a throw in `finally` replaces the in-flight exception, the caller saw "Cannot call write after a stream was destroyed" rather than the real cause. The fix replaces the `finally` close in both methods with a non-throwing pattern: skip `end()` entirely if `outputStream.destroyed` is already `true`; otherwise call `end(() => resolve())` — always resolving, never rejecting. A `if (streamError !== null) throw streamError` guard placed after the `finally` and before `atomicRename` then re-throws any captured stream error explicitly, preserving the real cause. New test (`src/__tests__/streaming-cleanup.test.ts`): `encryptFile` with a `makeErrorStream(2)` (fails on the first body chunk write) must surface an `ENOSPC`-containing `CryptoError` message, not one matching `ERR_STREAM_DESTROYED`.
- **`validatePath`: Windows drive-relative `..` bypass fixed** (`src/utils.ts`). `validatePath('C:..\\Windows\\foo')`, `validatePath('C:foo\\..\\..\\bar')`, and `validatePath('C:..')` all returned `{ isValid: true }` despite containing upward `..` traversal. The root cause: `path.normalize` on Windows keeps `..` glued to the drive specifier (`'C:..'` rather than a standalone `'..'`), and the previous drive-strip regex `/^[a-zA-Z]:$/` (strict exact match) did not match `'C:..'`, so `'C:..'` was passed to `checkSegments.includes('..')` — which uses strict equality and fails since `'C:..' !== '..'`. The fix changes the regex to the prefix-only `/^[a-zA-Z]:/` and strips the two-character drive prefix from `segments[0]` before the `includes('..')` scan: if stripping produces an empty string (the bare `'C:'` absolute-path case) the segment is dropped entirely, preserving the original behavior; if stripping leaves `'..'` or any other text it is kept as the new first element so the scan catches it. POSIX is unaffected — the entire block is gated behind `process.platform === 'win32'`. Reproduced against the live `dist/` before the fix (all three returned `isValid:true`); confirmed fixed after. Callers who pass `allowedRoot` were already protected by the resolved-prefix containment check; this fix closes the gap for callers who use `validatePath` without `allowedRoot`. New regression tests (`src/__tests__/utils.test.ts`): two new `it` blocks, both guarded `process.platform !== 'win32'`, asserting the three bypassed inputs are rejected with `error: 'Path traversal is not allowed'` and `C:\\Users\\foo` remains valid.
- **`encryptFileSync` plaintext chunk not scrubbed on error paths** (`src/crypto-manager.ts`). The 64 KiB plaintext reuse buffer (`chunk`) was declared `const` inside the `try` block and zeroed only on the success path. If an error occurred after `readSync` had populated the buffer (e.g. an `ENOSPC` or `EIO` on the `writeFileSync` call for the first body chunk), the `catch` block could not reference `chunk` and only scrubbed `key` — leaving up to 64 KiB of the caller's plaintext un-zeroed in heap until GC. The fix hoists `chunk` to `let chunk: Buffer | null = null` in the method's outer scope alongside `key`, `tempPath`, `inputFd`, and `outputFd`, and adds `if (chunk !== null) { this.secureClear(chunk); }` at the top of the `catch` block. The success-path scrub is unchanged. Scope is strictly limited to `encryptFileSync`'s plaintext buffer; `decryptFileSync`'s chunk holds ciphertext and is not modified. New test (`src/__tests__/streaming-cleanup.test.ts`): mock `writeFileSync` to fail on the first body-chunk write (so `readSync` has populated the buffer); assert `secureClear` was called with a 64 KiB `Buffer`.
- **`encryptFileSync` and `decryptFileSync` benign `closeSync(inputFd)` failure discarded valid output** (`src/crypto-manager.ts`). On the success path of both sync file methods, `closeSync(outputFd)` was followed immediately by bare `closeSync(inputFd)`. If the latter threw (rare `EBADF`/`EIO` on the read-only input fd, AFTER all crypto and output writing had succeeded and `outputFd` was already closed), the exception landed in the outer `catch`, which called `safeUnlinkSync(tempPath)` and deleted the complete, valid temp file — turning a benign close failure into total data loss without any completed work surviving. The fix wraps `closeSync(inputFd)` in a best-effort `try/catch` (with `inputFd = null` placed unconditionally after) in both methods, so only an `outputFd` close failure — which can signal unflushed or corrupt output — can now abort before the atomic rename. New tests (`src/__tests__/streaming-cleanup.test.ts`): (a) inject `closeSync` failure on the second call in `encryptFileSync`'s success path — the call must complete, the output must be readable; (b) inject `closeSync` failure on the fourth call across an `encryptFileSync` + `decryptFileSync` sequence (targeting `decryptFileSync`'s input fd close) — the decrypt must complete with the correct plaintext.
- **Constructor: `memoryCost < 8 * parallelism` now fails fast with `MEMORY_COST_TOO_SMALL`** (`src/crypto-manager.ts`). Argon2id requires `memoryCost >= 8 * parallelism` as a hard library constraint. Previously the constructor validated each option only against an upper cap, so a configuration such as `{ memoryCost: 4, parallelism: 1 }` constructed without error, then the first `encryptText` call threw a late, opaque `KEY_DERIVATION_FAILED ("Memory cost is too small")` from deep inside the KDF. This contradicts the library's fail-fast philosophy (it already validates `defaultPassphrase` at construction time). The fix computes the resolved (post-defaulting) `memoryCost` and `parallelism` after `this.argon2Options` is assigned and throws `CryptoError(INVALID_INPUT, 'MEMORY_COST_TOO_SMALL')` with an actionable message naming both values when the floor is violated. The defaults (`memoryCost = 2^17`, `parallelism = 1`) and all documented opt-down profiles (`2^16`, `2^14`, `2^12` with `p=1`) trivially pass. The sync PBKDF2 path ignores `memoryCost`, so the check is scoped to the Argon2 fields only. New error code: `MEMORY_COST_TOO_SMALL`. New tests (`src/__tests__/crypto-manager.test.ts`): under-boundary (`mc=7,p=1` and `mc=15,p=2`) → `CryptoError(MEMORY_COST_TOO_SMALL)`; at-boundary (`mc=8,p=1` and `mc=16,p=2`) and defaults → no throw.
- **Constructor: non-string `aad` option now throws `INVALID_AAD` instead of a raw `TypeError` or silent coercion** (`src/crypto-manager.ts`). `aad` was the only `CryptoManagerOptions` field with no runtime type guard. `Buffer.from(options.aad ?? '…', 'utf8')` silently coerced array inputs (e.g. `[72, 73]` → the bytes "HI"), and threw a raw Node `TypeError` (not a `CryptoError`) for numbers or plain objects — both violating the library's "all errors are CryptoError" contract and the documented `aad?: string` type. The fix adds an explicit `typeof` check before the `Buffer.from` call: if `options.aad` is defined but not a string, throw `CryptoError(INVALID_INPUT, 'INVALID_AAD')`. This reuses the existing `INVALID_AAD` code already used by `encryptData`/`decryptData` for `aadOverride`. New tests (`src/__tests__/crypto-manager.test.ts`): `aad: 123` (number) → `INVALID_AAD`; `aad: {}` (object) → `INVALID_AAD`; `aad: [72, 73]` (array, the silent-coercion regression case) → `INVALID_AAD`; `aad: 'custom-context'` (valid string) → no throw.
- **`src/types.ts` JSDoc: `memoryCost` default corrected from `65536` to `131072 — 2 ** 17, 128 MiB`** (`src/types.ts`). The IntelliSense tooltip for `CryptoManagerOptions.memoryCost` said `(default: 65536)` (2^16, 64 MiB) — the value from before the v0.15.0 HIGH-tier bump. The actual constructor default is `SECURITY_THRESHOLDS.HIGH.memoryCost = 2 ** 17 = 131072` (128 MiB). A consumer relying on the tooltip would have believed their instance defaults to 64 MiB / MEDIUM when the library actually uses 128 MiB / HIGH. All other documentation surfaces (README, CHANGELOG, `getSecurityLevel`) already reflect 2^17; only this JSDoc was stale. Runtime is unchanged; this is a doc-only correction.

- **Magic-collision recovery: v0 ciphertext whose salt begins with "HPCR" is now recoverable in auto mode** (`src/crypto-manager.ts`). A legacy v0 ciphertext has no header and starts with a 32-byte random salt. If the first 4 bytes of that salt happen to equal `"HPCR"` (~2^-32 per ciphertext), `hasMagic()` previously returned `true`, the code tried to parse a v1 header from the random salt bytes, and `parseHeader` threw `UNSUPPORTED_VERSION` / `UNSUPPORTED_KDF` / `INVALID_HEADER_PARAM` (depending on which random bytes were read). There was no recovery path, so the ciphertext became permanently undecryptable under any `legacyMode`. The fix adds a try/catch around ONLY `parseHeader` + `assertKdfMatches` in all four decrypt paths (`decryptText`, `decryptTextSync`, `decryptFile`, `decryptFileSync`). In the catch block: (a) if `legacyMode !== 'auto'`: re-throw the original parse error (preserving `UNSUPPORTED_VERSION` / `UNSUPPORTED_KDF` / `KDF_MISMATCH` for callers that switch on `err.code`, and correctly identifying future v2-format ciphertexts in non-auto modes); (b) if the error is `KDF_PARAMS_OUT_OF_BOUNDS`: always re-throw regardless of mode — this is a DoS-prevention error that must fire before any KDF work; (c) otherwise in `auto` mode: set `bodyOffset = 0` / `formatHeaderLen = 0`, leave `headerForAad = null` and argon/pbkdf2 overrides at their already-initialized v0 defaults, and continue with the v0 decrypt path. A genuine v1 ciphertext (produced by `packHeader`) always parses successfully, so the catch block is unreachable for any currently-succeeding decrypt call. After the fallback, GCM authentication is always performed and an incorrect password or tampered body is rejected with `DECRYPTION_FAILED`. Existing tests for tampered v1 headers (`UNSUPPORTED_VERSION`, `UNSUPPORTED_KDF`, `KDF_MISMATCH` in auto mode) were updated to reflect that auto mode now gets `DECRYPTION_FAILED` via the v0 fallback path; sub-tests with `legacyMode: 'strict'` verify the original error codes are still preserved in that mode. New tests (`src/__tests__/crypto-manager.test.ts`): four colliding-v0 round-trip tests (one per decrypt path, confirming `hasMagic() === true` on the fixture then verifying the plaintext is recovered in auto mode); two rejection tests (strict/reject mode still rejects the colliding v0 blob); one corrupt-v1 smoke test (valid magic + bad version byte still throws `CryptoError` in auto mode — via GCM auth failure).
- **Magic-collision recovery: unified the auto-mode re-throw predicate across all four decrypt paths** (`src/crypto-manager.ts`). The recovery above is documented — in the magic-collision entry directly above and in `decryptText`'s own in-catch comment — to re-throw exactly one error code unconditionally in `auto` mode, `KDF_PARAMS_OUT_OF_BOUNDS` (DoS protection), and to fall back to v0 for every other structural header-parse failure. Three of the four paths (`decryptText`, `decryptTextSync`, `decryptFileSync`) additionally re-threw `INVALID_HEADER_PARAM`, while `decryptFile` did not. As a result, a legacy v0 ciphertext whose random salt collided on `"HPCR"` **and** also formed a structurally-recognised v1 header with a zero-valued KDF parameter (so `parseHeader` throws `INVALID_HEADER_PARAM`) was recoverable via `decryptFile` but **not** via the text/sync paths — contradicting the documented contract, `decryptText`'s own adjacent comment ("one exception: `KDF_PARAMS_OUT_OF_BOUNDS`"), and the requirement that the recovery be identical in shape across all four paths. The fix removes the `|| headerParseErr.code === 'INVALID_HEADER_PARAM'` clause from those three predicates so all four re-throw only `KDF_PARAMS_OUT_OF_BOUNDS` and recover `INVALID_HEADER_PARAM` collisions as v0. This is safe: `INVALID_HEADER_PARAM` (a zero KDF parameter) is thrown at parse time before any KDF work, the v0 fallback uses fixed legacy/default KDF parameters (never the header's), and GCM authentication still rejects any wrong-key result with `DECRYPTION_FAILED`. New tests (`src/__tests__/crypto-manager.test.ts`): four zero-KDF-param collision round-trip tests (one per decrypt path, each asserting the fixture genuinely triggers `INVALID_HEADER_PARAM` via `parseHeader`, then recovers in auto mode) plus three corrupt-v1 "still throws" smoke tests covering `decryptTextSync`, `decryptFile`, and `decryptFileSync` (the prior such test covered only `decryptText`).
- **Test-hardening: file-path `KDF_MISMATCH` guard pinned in both directions** (`src/__tests__/crypto-manager.test.ts`). The sole file cross-KDF test (`encryptFileSync` PBKDF2 → `decryptFile` async Argon2id) previously asserted only `rejects.toThrow(CryptoError)` — a generic parse or size error would have satisfied it equally, so the actual `assertKdfMatches` call site was not confirmed to have fired. The test is now strengthened: a `legacyMode: 'strict'` instance is used alongside the auto-mode instance; the strict-mode assertion confirms `code === 'KDF_MISMATCH'`, proving the header was structurally valid and the KDF-id guard was the discriminating check. A mirror test — `encryptFile` async (Argon2id) → `decryptFileSync` sync (PBKDF2) — is added with the same auto + strict pattern, covering the `assertKdfMatches` call site in `decryptFileSync`. Both call sites (async and sync) are now independently pinned (audit #10). No production code changed.
- **Test-hardening: `encryptData`/`decryptData` `aadOverride` `INVALID_AAD` branch now covered** (`src/__tests__/crypto-manager.test.ts`). The guard `aadOverride !== undefined && !Buffer.isBuffer(aadOverride)` → `CryptoError(INVALID_INPUT, 'INVALID_AAD')` in both `encryptData` and `decryptData` was previously untested. A non-Buffer `aadOverride` (e.g. a string) would have reached the production guard and thrown, but no test verified the specific error code. Two new tests — one per method — assert `code === 'INVALID_AAD'` and `type === INVALID_INPUT` for a string `aadOverride`, pinning both guards (audit #11). No production code changed.

### Changed

- **`encryptText` and `encryptTextSync` now accept the empty string** (`src/crypto-manager.ts`). Previously both methods guarded with `if (!text || typeof text !== 'string')`, which used JavaScript's falsy shortcut and inadvertently rejected `''`. The library was therefore in an inconsistent state: `encryptFile` accepts 0-byte plaintext files, `decryptText` can return `''` for an empty-body v1 ciphertext, but `encryptText('')` threw `INVALID_TEXT` — a round-trip gap where you could decrypt what you could not produce. Two independent judges were summoned to decide between (a) relaxing the guard or (b) documenting the asymmetry; their verdicts split (judge A: relax; judge B: document), and the better-reasoned verdict (A) was chosen: the `!text` shorthand is a well-known JS anti-pattern conflating falsy with invalid, and empty-string encryption has legitimate use cases (optional fields, template systems). The fix changes the guard to the precise `if (text === undefined || text === null || typeof text !== 'string')`, and updates the error message from "Text must be a non-empty string" to "Text must be a string". `null` and `undefined` continue to throw `CryptoError(INVALID_INPUT, 'INVALID_TEXT')`. All existing callers passing non-empty strings are unaffected. New pinning tests (`src/__tests__/crypto-manager.test.ts`): `encryptText('', pw)` → `decryptText(enc, pw)` → `''` (async round-trip); `encryptTextSync('', pw)` → `decryptTextSync(enc, pw)` → `''` (sync round-trip); `null`/`undefined` inputs still throw with `code: 'INVALID_TEXT'`.

### Files changed

- `src/crypto-manager.ts` — `decryptFile`: add persistent `onStreamError` guard after `createWriteStream`; replace `finally` close with non-throwing robust close (`!destroyed` check + always-resolve `end(cb)`) + `removeListener`; add `if (streamError !== null) throw streamError` before `atomicRename`. `encryptFile`: same non-throwing `finally` close pattern (no change to `writeChunk` or `onStreamError` body); add `if (streamError !== null) throw streamError` before `atomicRename`. `encryptFileSync`: hoist `chunk` to outer scope; add `if (chunk !== null) { this.secureClear(chunk); }` in catch; wrap `closeSync(inputFd)` in best-effort try/catch. `decryptFileSync`: wrap `closeSync(inputFd)` in best-effort try/catch. Constructor: add `argon2MemFloor` check after `this.argon2Options` assignment; add `typeof options.aad !== 'string'` guard before `Buffer.from`. Decrypt paths: remove the `|| headerParseErr.code === 'INVALID_HEADER_PARAM'` clause from the auto-mode magic-collision re-throw predicate in `decryptText`, `decryptTextSync`, and `decryptFileSync` so all four paths match `decryptFile` (re-throw only `KDF_PARAMS_OUT_OF_BOUNDS`).
- `src/__tests__/streaming-cleanup.test.ts` — three new describe blocks for Phase 1: (7) Finding A regression (empty-body crash → clean CryptoError), (8) Finding B regression (body-write ENOSPC not masked by ERR_STREAM_DESTROYED), (9) async 0-byte file end-to-end round-trip; two new describe blocks for Phase 3: (10) `encryptFileSync` plaintext-chunk scrub regression, (11) benign `closeSync(inputFd)` failure does not discard completed work (both sync methods).
- `src/utils.ts` — `validatePath`: widen drive-strip regex from `/^[a-zA-Z]:$/` to `/^[a-zA-Z]:/`; strip the 2-char drive prefix from `segments[0]` before the `..` scan and only drop the segment when stripping leaves it empty; update JSDoc bullet to document drive-relative `..` coverage.
- `src/__tests__/utils.test.ts` — two new `it` blocks in the `validatePath` describe: (1) Windows drive-relative traversal bypass regression (three attack inputs rejected); (2) legitimate absolute Windows path sanity check (still valid).
- `src/__tests__/crypto-manager.test.ts` — four new `it` blocks in the `Constructor` describe: (1) `MEMORY_COST_TOO_SMALL` under-boundary; (2) at-boundary + defaults pass; (3) `INVALID_AAD` for number and object; (4) `INVALID_AAD` for array (silent-coercion regression); (5) valid string `aad` passes. Phase 5: `encryptText` `'should throw error for invalid text'` refactored to `'should throw INVALID_TEXT for null/undefined text'` with specific `code`/`type` assertions; new `'should encrypt and round-trip the empty string'` async test; `encryptTextSync` parallel changes (sync `'should throw INVALID_TEXT for null/undefined text'` + `'should encrypt and round-trip the empty string (sync)'`). Phase 7 (test-hardening): `encryptData` describe — new `'should throw INVALID_AAD when aadOverride is not a Buffer'` test; `decryptData` describe — matching test; `encryptFileSync` cross-KDF describe — existing `'cross-KDF: decryptFile'` test strengthened with strict-mode `code === 'KDF_MISMATCH'` assertion; new mirror test `'cross-KDF: decryptFileSync (sync/PBKDF2) correctly rejects encryptFile (async/Argon2id) ciphertext'` with auto + strict pattern. Magic-collision describe: four new zero-KDF-param recovery `it` blocks (one per decrypt path, each asserting the fixture genuinely triggers `INVALID_HEADER_PARAM` then recovers in auto mode) and three new corrupt-v1 "still throws" smoke tests for `decryptTextSync`/`decryptFile`/`decryptFileSync`.
- `src/__tests__/property.test.ts` — corrected two stale comments that claimed `CryptoManager` rejects empty strings (no longer true after the empty-string change above); the `minLength=1` text arbitrary itself is unchanged (empty-string plaintext is covered by dedicated unit tests).
- `README.md` — corrected the `encryptFileSync` progress-callback rationale (it no longer "reads the entire input in a single `readFileSync` call" after the v1.4.0 chunked rewrite; it streams in fixed 64 KiB chunks but intentionally emits no per-chunk progress events) in both the method description and the progress-contract table.
- `src/types.ts` — `CryptoManagerOptions.memoryCost` JSDoc: `(default: 65536)` → `(default: 131072 — 2 ** 17, 128 MiB)`.
...
- `src/__tests__/crypto-manager.test.ts` — 8 new constructor-cap tests; 8 new key-scrub error-path tests; 4 new non-string path argument tests; 5 new `encryptFileSync` streaming round-trip tests; 10 wrong-password/tamper tests strengthened with `.code`/`.type` assertions.
- `src/__tests__/streaming-cleanup.test.ts` — 4 new `outputStream` error-injection tests (prefix/tag × immediate/backpressured); 1 new short front-matter read test for `decryptFile`.
...
- `src/__tests__/property.test.ts` — sync-file round-trip property added to `round-trip invariants`.
- `README.md` — TypeScript version badge `5.8.3` → `6.0.3`.
...
- **91 × `js/insecure-temporary-file`** — dismissed as `used in tests` (87) or `false positive` (4).
  - The 87 test alerts flag the documented per-suite scratch-directory pattern: `path.join(os.tmpdir(), 'hiprax-crypto-${randomBytes(8).hex}')`. 64 bits of entropy in the suffix make CWE-377-style pre-existing-file collision infeasible; the pattern is the project's standard testing convention.
...
- `.github/PULL_REQUEST_TEMPLATE.md` (new) — security-impact callout and pre-completion checklist mirroring the project's quality gates.
...
- **`README.md`**: No structural changes — the new tests are additive and follow existing conventions (`os.tmpdir()` for scratch dirs, async/sync split, `expect(...).rejects.toThrow(CryptoError)` for async rejection).
