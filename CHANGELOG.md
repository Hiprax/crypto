...
- **Documentation synchronised with the real `CryptoManager` API surface** (`README.md`, `CLAUDE.md`). The README constructor Options list was missing `skipPasswordValidation` and `legacyHeaderAad` (both were already described in prose elsewhere); both are now listed with cross-links to the Password Requirements and v1.0.0 → v1.1.0 Migration sections. The README Methods section and CLAUDE.md's Methods lists gained entries for `getLegacyMode(): LegacyMode` and `inspectHeader(input: string | Buffer): ParsedHeader | null` (documenting the `null`-for-v0 return and the `INVALID_BASE64URL` fail-fast for malformed strings, cross-linking the existing worked example rather than duplicating it). CLAUDE.md's Constructor Options table gained the missing `legacyHeaderAad` row. Docs-only; no code or behavior change.
