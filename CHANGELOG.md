...
- `src/format.ts` — both new symbols added to its named re-export list, so the Node and browser surfaces stay symmetric.
...
- `README.md` — the three static browser gates and `npm run verify` documented, `DATA_TOO_LARGE_FOR_GCM` and the GCM bound added to the error-handling section, `CONTAINER_VERSION` / "Telling the formats apart" added to the format documentation, `inspectHeader` documented as a v1 inspector that throws on a container, `EncryptionResult` noted as Node-only.
