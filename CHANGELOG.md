...
- `src/__tests__/crypto-manager.test.ts` — four new `it` blocks in the `Constructor` describe: (1) `MEMORY_COST_TOO_SMALL` under-boundary; (2) at-boundary + defaults pass; (3) `INVALID_AAD` for number and object; (4) `INVALID_AAD` for array (silent-coercion regression); (5) valid string `aad` passes.
